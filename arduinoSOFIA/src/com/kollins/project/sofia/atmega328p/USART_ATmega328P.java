...
//                Log.d(USART_LOG_TAG, "Transmitting: " + ((char) transmitter_UDR0));
        ...
        if (dataMemory.readBit(DataMemory_ATmega328P.UCSR0B_ADDR, 4)) {
            ...
            if (!dataMemory.readBit(DataMemory_ATmega328P.UCSR0A_ADDR, 7)
            		 && UCModule.serialBuffer.length() > 0) {//   && SerialFragment.buffer.length() > 0) {

                receiver_UDR0 = UCModule.serialBuffer.substring(0, 1).getBytes()[0];
                UCModule.serialBuffer = UCModule.serialBuffer.substring(1);
