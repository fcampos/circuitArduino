...
public class TimerElm extends ChipElm {
    ...
    final int N_OUT = 5;
    final int N_RST = 6;
    protected int getDefaultFlags() { return FLAG_RESET; }
	...
	sim.algorithm.stampResistor(nodes[N_VIN], nodes[N_CTL],  5000);
	sim.algorithm.stampResistor(nodes[N_CTL], 0,        10000);
	// output pin
	sim.algorithm.stampVoltageSource(0, nodes[N_OUT], pins[N_OUT].voltSource);
	// discharge pin
	sim.algorithm.stampNonLinear(nodes[N_DIS]);
    }
	...
	if (!out && !setOut)
	    sim.algorithm.stampResistor(nodes[N_DIS], 0, 10);
	// output
	sim.algorithm.updateVoltageSource(0, nodes[N_OUT], pins[N_OUT].voltSource,
			    out ? volts[N_VIN] : 0);
    }
    protected int getPostCount() { return hasReset() ? 7 : 6; }
