...
//import circuitMatlab.components.chips.ChipElm.Pin;
  ...
  public  class VCOElm extends ChipElm {
	...
	protected void stamp() {
	    // output pin
	    sim.algorithm.stampVoltageSource(0, nodes[1], pins[1].voltSource);
	    // attach Vi to R1 pin so its current is proportional to Vi
	    sim.algorithm.stampVoltageSource(nodes[0], nodes[4], pins[4].voltSource, 0);
	    // attach 5V to R2 pin so we get a current going
	    sim.algorithm.stampVoltageSource(0, nodes[5], pins[5].voltSource, 5);
	    // put resistor across cap pins to give current somewhere to go
	    // in case cap is not connected
	    sim.algorithm.stampResistor(nodes[2], nodes[3], cResistance);
	    sim.algorithm.stampNonLinear(nodes[2]);
	    sim.algorithm.stampNonLinear(nodes[3]);
	}
	...
	protected void doStep() {
	    ...
	    sim.algorithm.updateVoltageSource(0, nodes[1], pins[1].voltSource, vo);
	    ...
	    sim.algorithm.stampMatrix(nodes[2], cur1, dir);
	    sim.algorithm.stampMatrix(nodes[2], cur2, dir);
	    sim.algorithm.stampMatrix(nodes[3], cur1, -dir);
	    sim.algorithm.stampMatrix(nodes[3], cur2, -dir);
	    cDir = dir;
	}
