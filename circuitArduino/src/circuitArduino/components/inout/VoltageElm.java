...
public class VoltageElm extends CircuitElm {
    ...
    double frequency,  freqTimeZero, bias,
	...
	if (waveform == WF_DC)
	    sim.algorithm.stampVoltageSource(nodes[0], nodes[1], voltSource,
			       getVoltage());
	else
	    sim.algorithm.stampVoltageSource(nodes[0], nodes[1], voltSource);
    }
	...
	if (waveform != WF_DC)
	    sim.algorithm.updateVoltageSource(nodes[0], nodes[1], voltSource,
				getVoltage());
    }
    double getVoltage() {
