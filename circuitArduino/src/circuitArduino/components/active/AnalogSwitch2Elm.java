...
public class AnalogSwitch2Elm extends AnalogSwitchElm {
	...
	if ((flags & FLAG_INVERT) != 0)
	    open = !open;
	if (open) {
	    sim.algorithm.stampResistor(nodes[0], nodes[2], r_on);
	    sim.algorithm.stampResistor(nodes[0], nodes[1], r_off);
	} else {
	    sim.algorithm.stampResistor(nodes[0], nodes[1], r_on);
	    sim.algorithm.stampResistor(nodes[0], nodes[2], r_off);
	}
    }
