...
class TriacElm extends CircuitElm {
    ...
    protected int getDumpType() { return 183; }
	...
	sim.algorithm.stampNonLinear(nodes[anode]);
	sim.algorithm.stampNonLinear(nodes[cnode]);
	sim.algorithm.stampNonLinear(nodes[gnode]);
	sim.algorithm.stampNonLinear(nodes[inode]);
	sim.algorithm.stampResistor(nodes[gnode], nodes[cnode], cresistance);
	...
	sim.algorithm.stampResistor(nodes[anode], nodes[inode], aresistance);
    }
