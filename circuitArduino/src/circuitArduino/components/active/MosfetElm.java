...
import circuitArduino.UI.EditInfo;
 ...
 public   class MosfetElm extends CircuitElm {
	...
	int hs = sim.gridSize*2;//  final int hs = sim.gridSize*2;//16;
	...
	protected void setPoints() {
	    ...
	    interpPoint2(point1, point2, src[1], drn[1], 1-2.7*sim.gridSize/dn, -hs2); //1-22/dn
	    interpPoint2(point1, point2, src[2], drn[2], 1-2.7*sim.gridSize/dn, -hs2*4/3); //1-22/dn

	    gate = newPointArray(3);
	    interpPoint2(point1, point2, gate[0], gate[2], 1-3.5*sim.gridSize/dn, hs2/2); // was 1-20/dn //28/dn
	...
	protected void doStep() {
	    ...
	    sim.algorithm.stampMatrix(nodes[drain],  nodes[drain],  Gds);
	    sim.algorithm.stampMatrix(nodes[drain],  nodes[source], -Gds-gm); 
	    sim.algorithm.stampMatrix(nodes[drain],  nodes[gate],   gm);
	    
	    sim.algorithm.stampMatrix(nodes[source], nodes[drain],  -Gds);
	    sim.algorithm.stampMatrix(nodes[source], nodes[source], Gds+gm); 
	    sim.algorithm.stampMatrix(nodes[source], nodes[gate],  -gm);
	    
	    sim.algorithm.stampRightSide(nodes[drain],  rs);
	    sim.algorithm.stampRightSide(nodes[source], -rs);
	    if (source == 2 && pnp == 1 ||
		source == 1 && pnp == -1)
		ids = -ids;
	}
