...
import circuitArduino.UI.EditInfo;
 ...
 public   class OpAmpElm extends CircuitElm {
	...
	void setSize(int s) {
	    opsize = s;
	    opheight = sim.gridSize*s;//8*s;
	    opwidth = (int)Math.round(1.5*(float)(sim.gridSize))*s;//13*s;
	    flags = (flags & ~FLAG_SMALL) | ((s == 1) ? FLAG_SMALL : 0);
	}
	protected void setPoints() {
	   int s;
		super.setPoints();
	    System.out.println(Double.toString(dn));
	    System.out.println(sim.gridSize*15);
	    if (dn > sim.gridSize*15 && this == sim.dragElm)//(dn > 120 && this == sim.dragElm)
	    {	setSize(2);
	    System.out.println("setting double size");
	    }
	...
	protected void stamp() {
	    ...
	    sim.algorithm.stampMatrix(nodes[2], vn, 1);
	}
	protected void doStep() {
	    ...
	    // newton-raphson
	    sim.algorithm.stampMatrix(vn, nodes[0], dx);
	    sim.algorithm.stampMatrix(vn, nodes[1], -dx);
	    sim.algorithm.stampMatrix(vn, nodes[2], 1);
	    sim.algorithm.stampRightSide(vn, x);
