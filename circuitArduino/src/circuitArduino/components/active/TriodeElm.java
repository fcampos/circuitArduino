...
public class TriodeElm extends CircuitElm {
	...
	currentg = 0;
	if (vgk > .01) {
	    sim.algorithm.stampResistor(nodes[grid], nodes[cath], gridCurrentR);
	...
	sim.algorithm.stampMatrix(nodes[plate],  nodes[plate],  Gds);
	sim.algorithm.stampMatrix(nodes[plate],  nodes[cath],  -Gds-gm); 
	sim.algorithm.stampMatrix(nodes[plate],  nodes[grid],   gm);
	    
	sim.algorithm.stampMatrix(nodes[cath],   nodes[plate],  -Gds);
	sim.algorithm.stampMatrix(nodes[cath],   nodes[cath],    Gds+gm); 
	sim.algorithm.stampMatrix(nodes[cath],   nodes[grid],   -gm);
	    
	sim.algorithm.stampRightSide(nodes[plate],  rs);
	sim.algorithm.stampRightSide(nodes[cath ], -rs);
    }
