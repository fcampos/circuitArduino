...
public class LinAxisPotElm extends CircuitElm implements AdjustmentListener {
    ...
    Label label;
    String sliderText;
    String motorName = "undefined";
    DCmotorElm motor;
    double prevMotorAngle;
    
    public LinAxisPotElm(int xx, int yy) {
	super(xx, yy);
	noDiagonal = true;
	setup();
	maxResistance = 1000;
	position = .5;
	sliderText = "Linear Ax.";
	createSlider();
    }
    public LinAxisPotElm(int xa, int ya, int xb, int yb, int f,
		  StringTokenizer st) {
	super(xa, ya, xb, yb, f);
	maxResistance = new Double(st.nextToken()).doubleValue();
	position = new Double(st.nextToken()).doubleValue();
	motorName = st.nextToken();
	sliderText = st.nextToken();
	
	while (st.hasMoreTokens())
	    sliderText += ' ' + st.nextToken();
	createSlider();
	searchExistingMotors();
    }
    void setup() {
    }
	...
	//int segments = 16+4;
	if (abs(dx) > abs(dy)) {
	    dx = sign(dx)*sim.gridSize*12;//sim.snapGrid(dx/2,true)*2;
	    point2.x = x2 = point1.x + dx;
	  //  offset = (dx < 0) ? dy : -dy;
	    point2.y = point1.y;
	} else {
	    dy = sign(dy)*sim.gridSize*12;//sim.snapGrid(dy/2,false)*2;
...
//	System.out.println("arrow2 " + arrow2.x + " " +arrow2.y);
	...
	drawCenteredText(g, sliderText, textPoint.x, textPoint.y,true);
	draw2Leads(g);
	setPowerColor(g, true);
	
	
	// ########################################### DRAW LINEAR AXIS ANIMATION -----------------------------------------------
	...
	/*if (motor!=null)
	{
		...
		currentMotorAngle = motor.angle;//*.001;
		increment =  currentMotorAngle - prevMotorAngle;
		if (axisPos + increment<=1 & axisPos + increment>=0)
			{axisPos += increment*.01;
	...
	g.fillRect(axisX0, axisY0, axisLen, axisWidth);
	g.setColor(Color.ORANGE);
	g.fillRect(axisX0 + (int)(axisPos*axisLen), axisY0-15, 20, 10);
    ...
    void updateFromMotor()
    {
    	...
    	if (motor!=null)
    	{
    		...
    		currentMotorAngle = motor.angle;//*.001;
    		increment =  (currentMotorAngle - prevMotorAngle)*.01;
    		if (axisPos + increment<=1 & axisPos + increment>=0)
    			{axisPos += increment;
    			prevMotorAngle = currentMotorAngle;
    			}
    ...
    void searchExistingMotors()
    {
    	DCmotorElm foundMotor;
    	int i;
        for (i = 0; i != sim.elmList.size(); i++) {
		    CircuitElm ce = sim.getElm(i);
		    if (ce instanceof DCmotorElm)
		    {  foundMotor = (DCmotorElm)(ce);
		    System.out.println("found motor");
		    System.out.println(motorName);
		    System.out.println(foundMotor.name);
	    if (motorName.equals(foundMotor.name))
	    {	motor = foundMotor;
	    System.out.println("matched motor");
	    }
	    }
	    }
    }
