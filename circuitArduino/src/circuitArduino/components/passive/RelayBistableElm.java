...
public class RelayBistableElm extends CircuitElm {
    double inductance;
    Inductor ind;
    //double inductance2;
    Inductor ind2;
    ...
    Point lines[];
    double coilCurrent,coilCurrent2, switchCurrent[], coilCurCount, coilCurCount2, switchCurCount[];
    ...
    int nCoil2_1, nCoil2_2, nCoil2_3;
    final int FLAG_SWAP_COIL = 1;
    
    public RelayBistableElm(int xx, int yy) {
	super(xx, yy);
	...
	coilCurrent2 = coilCurCount2 =0;
	poleCount = 1;
	setupPoles();
    }
    public RelayBistableElm(int xa, int ya, int xb, int yb, int f,
		    StringTokenizer st) {
	super(xa, ya, xb, yb, f);
	...
	nCoil2_1 = 3+3*poleCount;
	nCoil2_2 = 3+nCoil1+1;
	nCoil2_3 = 3+nCoil1+2;
	
	nCoil1 = 3*poleCount;
	nCoil2 = 3*poleCount+1;
	nCoil3 = 3*poleCount+4;
	
	nCoil2_1 = 2+3*poleCount;
	nCoil2_2 = 3+3*poleCount;
	nCoil2_3 = 5+3*poleCount;
	
	
	System.out.println(nCoil1+" "+nCoil2+" "+nCoil3+" "+nCoil2_1+" "+nCoil2_2+" "+nCoil2_3+" ");
	...
	setVoltageColor(g, volts[nCoil2_1]);
	 drawThickLine(g, coilLeads[2], coilPosts[2]);
	 setVoltageColor(g, volts[nCoil2_1+1]);
	 drawThickLine(g, coilLeads[3], coilPosts[3]);
	int x = ((flags & FLAG_SWAP_COIL) != 0) ? 1 : 0;
	drawCoil(g, dsign*6, coilLeads[x], coilLeads[1-x],
		 volts[nCoil1+x], volts[nCoil2-x]);
	drawCoil(g, dsign*6, coilLeads[x+2], coilLeads[1-x+2],
			 volts[nCoil2_1+x], volts[nCoil2_2-x]);
	...
	coilCurCount2 = updateDotCount(coilCurrent2, coilCurCount2);
	...
	drawDots(g, coilPosts[2], coilLeads[2], coilCurCount2);
	drawDots(g, coilLeads[2], coilLeads[3], coilCurCount2);    
	drawDots(g, coilLeads[3], coilPosts[3], coilCurCount2);
	
	drawPosts(g);
	setBbox(coilPosts[0], coilLeads[3], 0);
	...
	interpPoint(point1, point2, coilPosts[2],  x, openhs*4);
	interpPoint(point1, point2, coilPosts[3],  x, openhs*5);
    ...
    protected int getPostCount() { return 2+poleCount*3+2; }
	...
	ind2.stamp(nodes[nCoil2_1], nodes[nCoil2_3]);
	// resistor from internal node to coil post 2
	sim.algorithm.stampResistor(nodes[nCoil3], nodes[nCoil2], coilR);
	sim.algorithm.stampResistor(nodes[nCoil2_3], nodes[nCoil2_2], coilR);

	int i;
	for (i = 0; i != poleCount*3; i++)
	    sim.algorithm.stampNonLinear(nodes[nSwitch0+i]);
    }
    protected void startIteration() {
	ind.startIteration(volts[nCoil1]-volts[nCoil3]);
	ind2.startIteration(volts[nCoil2_1]-volts[nCoil2_3]);

	// magic value to balance operate speed with reset speed semi-realistically
	double magic = 1.3;
	double pmult = Math.sqrt(magic+1);
	double p = coilCurrent/onCurrent;//coilCurrent*pmult/onCurrent;
	double p2 = coilCurrent2/onCurrent;//coilCurrent2*pmult/onCurrent;
	
	double aux;
	if (Math.abs(p2)>.1|Math.abs(p)>.1){
		if (Math.abs(p2)<Math.abs(p)){
			aux = 1- Math.abs(p) ;
			if (aux<d_position)//{
				d_position = aux;
			//System.out.println("p "+d_position+"  "+ aux);}
			else if(aux<.5)
				d_position=.8*d_position;
			//d_position = 1-Math.abs(p*p) - 1.3;
		}
		else{
			aux = Math.abs(p2);
			if (aux>d_position)//{
				d_position = aux;
				//System.out.println("p2 "+d_position+"  "+ aux);
		//	}
			else if(aux>.5)
				d_position=1.2*d_position;
		}
		if (d_position < 0)
			d_position = 0;
		if (d_position > 1)
		...
		if (i_position==2){
			if (d_position < .5){d_position = 0;i_position = 0;}
			if (d_position >= .5){d_position = 1;i_position = 1;}
		}
	}
    }
	...
	ind.doStep(voltdiff);
	double voltdiff2 = volts[nCoil2_1]-volts[nCoil2_3];
	ind2.doStep(voltdiff2);
	int p;
	for (p = 0; p != poleCount*3; p += 3) {
	    sim.algorithm.stampResistor(nodes[nSwitch0+p], nodes[nSwitch1+p],
			      i_position == 0 ? r_on : r_off);
	    sim.algorithm.stampResistor(nodes[nSwitch0+p], nodes[nSwitch2+p],
	...
	arr[ln++] = "coil2 I = " + getCurrentDText(coilCurrent2);
	arr[ln++] = "coil2 Vd = " +
	    getVoltageDText(volts[nCoil2_1] - volts[nCoil2_2]);
    }
	...
	if (n == 0 && ei.value > 0) {
	    inductance = ei.value;
	    ind.setup(inductance, coilCurrent, Inductor.FLAG_BACK_EULER);
	    ind2.setup(inductance, coilCurrent2, Inductor.FLAG_BACK_EULER);
	}
	if (n == 1 && ei.value > 0)
	    r_on = ei.value;
	if (n == 2 && ei.value > 0)
